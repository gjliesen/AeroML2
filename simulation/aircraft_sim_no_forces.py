...
class AircraftSimNoForces(Aircraft_Sim):
    def __init__(self, initial_conditions):
        ...
        g_n = 9.81
        super().__init__(x_E_E_BE_0, att_B_B_BN_0, v_E_B_BE_0, w_B_B_BE_0, Fext_B_0, Mext_B_0, g_n)
        self.lla_states_init = np.hstack((self.p_E_E_BE_0, self.states_init[3:7],
                                          v_E_B_BE_0, w_B_B_BE_0))
    def get_all_states(self):
        x_E_E_BE = self.x_E_E_BE_states
        lat, lon, alt = navpy.ecef2lla(x_E_E_BE, latlon_unit='deg')
        p_E_E_BE = np.hstack((lat.reshape(-1, 1), lon.reshape(-1, 1), alt.reshape(-1, 1)))
